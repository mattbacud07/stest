<template>
    <v-btn type="button" :disabled="btnDisable" color="primary" variant="flat" class="text-none btnSubmit"><v-icon
            ...
            <v-card text="" title="Delegate Engineer" prepend-icon="mdi-account">
                <v-form @submit.prevent="delegateEngineer" ref="engineerForm">
                        ...
                        <v-combobox color="primary" class="mt-5" v-model="Engineers" clearable label="Delegate to"
                            ...
                            <template v-slot:selection="{ item, index }">
                                <span class="small text-primary" style="font-size: 12px;"><b>{{
                                    item.title }}</b> <span class="text-disabled" v-if="item.title">- [SBU : {{
                    ...
                    <v-row justify="end" class="mt-7 mb-5 pr-3">
                        <v-btn variant="plain" @click="engineerDialog = false" color="primary" class="text-none mr-2">
                            Cancel</v-btn>
...
const delegateEngineer = async () => {
    ...
    try {
        const res = await apiRequest.post('pr_delegate_engineer', {
            id: props.id,
