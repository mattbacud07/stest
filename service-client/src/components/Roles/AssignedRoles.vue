<template>
            <v-col cols="12" style="background: #3333330d;border-radius: 3px;">
                <v-row>
                    <v-col cols="7">
                        <v-btn color="primary" size="small" class="text-none mr-2" :disabled="btnDisabledUpdate"><v-icon>mdi-file-document-edit</v-icon> Edit</v-btn>
                        <v-btn color="error" size="small" class="text-none" :disabled="btnDisabledDelete"><v-icon>mdi-delete-empty</v-icon> Delete</v-btn>
                    </v-col>
                    <v-col cols="5">
                ...
                <vue3-datatable ref="datatable"  :rows="rows" :columns="cols" :loading="loading" :search="params.search"
                   :selectRowOnClick="true" :hasCheckbox="true" :sortable="true"
                    :hide="true" :filter="true" skin="bh-table-compact bh-table-bordered" @rowSelect="rowSelect">
                </vue3-datatable>
            </v-col>
</template>
<script setup>
