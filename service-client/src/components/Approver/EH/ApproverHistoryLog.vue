    ...
    <v-card v-else>
        <v-timeline density="comfortable" align="start" side="end" line-thickness="1">
            <template v-for="(detail, index) in details" :key="index">
                <v-timeline-item v-if="detail.level === pub_var.INSTALLATION_TL" size="md" dot-color="grey">
                ...
                <v-timeline-item v-else size="md" dot-color="grey">
                    ...
                    <!-- Internal Processing -->
                    <div v-if="detail.level === pub_var.SERVICE_TL && props.status === pub_var.INTERNAL_SERVICING"
                        class="mt-4">
                        <v-row>
                            <v-col>
