...
import { A_CM, A_EH, A_PM, PM } from '@/global/modules';
...
const breadcrumbItems = [
    { title: 'Back', disabled: false, href: '/corrective-maintenance', display: 'block' },
    { title: 'Corrective Maintenance', disabled: true, href: '', display: 'none' },
    { title: 'CM-Details', disabled: true, href: '', display: 'none' },
]
...
const getPMDataByRowID = async () => {
    loading.value = true
    try {
        const response = await apiRequest.get('get_cm_record_details', {
            params: {
                id: id,
                module_type: A_CM
            }
        ...
        if (response?.data?.cm) {
            const result = response.data.cm
            pm_data.value = response.data.cm
...
const markAsCompleted = async () => {
    ...
    try {
        const response = await apiRequest.post('cm_mark_as_completed', {
            id: id,
            delegation_id: task_delegation.value?.id,
