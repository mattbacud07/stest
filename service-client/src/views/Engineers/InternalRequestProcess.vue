        ...
        <template #topBarFixed>
            ...
            <div v-if="role.currentUserRole === pub_var.engineerRole">
                <span v-if="status === pub_var.internalStat.Delegated">
                    <v-dialog v-model="dialog" max-width="400" persistent>
                        <template v-slot:activator="{ props: activatorProps }">
                            <v-btn :disabled="btnDisable" v-bind="activatorProps" size="small" color="white"
                            ...
                            <template v-slot:actions>
                                <v-row justify="end">
                                    <v-btn elevation="2" @click="dialog = false" background-color="red" size="small"
                                        ...
                                        Cancel</v-btn>
                                    <v-btn @click="declineTask" color="primary" elevation="2" size="small"
                    ...
                    <!-- Approve Button -->
                    <v-btn type="button" :loading="btnSubmitLoading" density="compact" @click="acceptInternalRequest"
                        :disabled="btnDisable" color="warning" variant="flat" class="text-none btnSubmit"><v-icon
                ...
                <span v-if="status === pub_var.internalStat.InProgress">
                    ...
                    <v-dialog v-model="dialog" max-width="400" persistent>
                        <template v-slot:activator="{ props: activatorProps }">
                            <v-btn type="button" v-bind="activatorProps" :loading="btnSubmitLoading" size="small"
                                :disabled="btnDisable" color="warning" variant="flat"
                            ...
                            <template v-slot:actions>
                                <v-row justify="end">
                                    <v-btn elevation="2" @click="dialog = false" background-color="red" size="small"
                                        color="#191970" class="text-none mr-2"><v-icon>mdi-close</v-icon>
                                        Cancel</v-btn>
                                    <v-btn @click="markAsCompleted" color="primary" elevation="2" size="small"
                ...
                <span v-if="status === pub_var.internalStat.Completed">
                    <!-- Approve Button -->
                    <v-btn type="button" :loading="btnSubmitLoading" size="small" @click="markAsPackedEndorsed"
                        :disabled="btnDisable" color="warning" variant="flat" class="text-none btnSubmit"><v-icon
                            class="mr-2">mdi-package-check</v-icon>
...
const markAsCompleted = async () => {
    ...
    try {
        ...
        if (response.data && response.data.success) {
            messageDetails.value = { show: true, color: 'success', text: 'Operation completed successfully' }
            getInternalRequest()
            refreshKey.value++
            // setTimeout(() => {
            //     // router.push('/internal-servicing')
