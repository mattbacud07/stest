                        ...
                        <v-row>
                            ...
                            <v-col cols="8" style="text-align: right;">
                                <!-- <router-link :to="{ name: 'ViewWorkOrder', params: { id: selectedId ?? 0 } }"> -->
                                    <v-btn @click="acceptTask" :disabled="btnDisable" type="button" color="primary" class="text-none mr-2">
                                        <v-icon class="mr-2">mdi-file-check-outline</v-icon> Accept
                                    </v-btn>
                                <!-- </router-link> -->
...
const rowSelect = async () => {
    ...
    if (selectedRows && selectedRows.length === 1 && rows.value[0].status === 'Pending Acceptance') {
...
/** Accepting Task for PM */
const acceptTask = async() => {
    try {
        const response = await apiRequest.post('accept-pm-task',{
            id : selectedId.value
    })
    if(response.data && response.data.success){
        messageDetails.value = { show: true, color: 'success', text: 'Successfully Accepted' }
        setTimeout(()=>{
            router.push('/pm-sched-accepted/servive_id='+ selectedId.value)
        }, 1000)
    }else{
