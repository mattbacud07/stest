                    ...
                    <v-card class="mx-auto p-4">
                        <v-row>
                            ...
                            <v-col cols="8" style="text-align: right;">
                                ...
                                <router-link :to="{ name: 'AssignedEquipmentInstallation', params: { id: selectedId ?? 0, status : status ?? 0 } }">
...
const getAssignedRequest = async () => {
    try {
        loading.value = true;
        const response = await apiRequest.get('get-assigned-request',{
            params : { id : user.user.id}
        });
        
        if(response.data && response.data.assigned_data){
            const data = response.data.assigned_data
            rows.value = data
        }else{
