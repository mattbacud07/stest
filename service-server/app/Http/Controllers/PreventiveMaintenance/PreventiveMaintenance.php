...
class PreventiveMaintenance extends Controller
{
    ...
    public function get_preventive_maintenance(Request $request)
    {
        ...
        try {
            ...
            $query = PM::select(
                ...
                ->leftJoin('service_master_data as smd', 'preventive_maintenance.item_id', '=', 'smd.id')
                ->leftJoin(DB::connection('mysqlSecond')->getDatabaseName() . '.master_data as m', 'smd.master_data_id', '=', 'm.id')
                ->leftJoin(DB::connection('mysqlSecond')->getDatabaseName() . '.mt_bp_institutions as i', 'preventive_maintenance.institution', '=', 'i.id')
                ->leftJoin(DB::connection('mysqlSecond')->getDatabaseName() . '.users as u_by', 'preventive_maintenance.delegated_by', '=', 'u_by.id')
                ->leftJoin(DB::connection('mysqlSecond')->getDatabaseName() . '.users as u_to', 'preventive_maintenance.delegated_to', '=', 'u_to.id');
            ...
            if ($this->role_validator->userHasRole($current_role)) {
                        ...
                        $subQuery->where(function ($q) use ($tl_assitant_sbu) {
                            $q->whereExists(function ($existQuery) use ($tl_assitant_sbu) {
                                $existQuery->select(DB::raw(1))
                                    ...
                                    ->whereColumn('smd.id', '=', 'preventive_maintenance.item_id');
                            });
                        });
                    } elseif ($current_role === Roles::engineerRoleID) {
                        $subQuery->where('preventive_maintenance.delegated_to', '=', $user_id);
                    }
                });
                ...
                if ($request->has('filterStatus')) {
                    ...
                    $query->whereIn('preventive_maintenance.status', $status);
                }
                if ($request->has('assignment')) {
                    $assignment = filter_var($request->assignment, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE);
                    if ($assignment === true) {
                        $lastDayOfNextMonth = Carbon::today()->addMonth()->endOfMonth();
                ...
                if ($request->has('filterInstitution')) {
                    ...
                    $query->whereIn('preventive_maintenance.institution', $institutions);
                }
    ...
    {
        return $this->soft_delete_data(PM::class, $id);
    }
}
